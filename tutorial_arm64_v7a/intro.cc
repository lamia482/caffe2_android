...
void run() {
  ...
  for (auto op : gradient_ops) {
    ...
    // std::cout << "Z3, output size: " << output.size() << "\n";
    for (auto i = 0; i < output.size(); i++) {
      output[i].dense_ = op->output(i) + "_grad";
      // std::cout << output[i].dense_ << "\n";
    }
  ...
  // initModel.mutable_device_option()->set_device_type(OPENGL);
  // predictModel.mutable_device_option()->set_device_type(OPENGL);
  ...
  // std::cerr << "Now create a GLPredictor object\n";
  // GLPredictor *glp = new GLPredictor(initModel, predictModel);
  // glp.run()
  // delete glp;
  // std::cerr << "Now destroy a GLPredictor object\n";
